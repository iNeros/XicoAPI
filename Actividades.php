...
function permisos() {  
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS'){
    ...
    exit(0);
        ...
        $sql = "SELECT * FROM actividades WHERE  id_docente = '".$_GET['id_docente']."' ORDER BY id_actividad desc";
        $resultado = mysqli_query($conexion,$sql) or die ( "Algo ha ido mal en la consulta a la   base de datos");
        $datos = mysqli_fetch_all($resultado,MYSQLI_ASSOC);
        header("HTTP/1.1 200 OK");
        echo json_encode($datos);
        ...
        $sql = "SELECT MAX(id_actividad) AS nuevo_id FROM `actividades`";
        ...
        $sql = "SELECT * FROM `actividades` WHERE id_actividad = '".$_GET['id_actividad']."'";
        $resultado = mysqli_query($conexion,$sql) or die ( "Algo ha ido mal en la consulta a la   base de datos act");
        $datos = mysqli_fetch_all($resultado,MYSQLI_ASSOC);
        header("HTTP/1.1 200 OK");
        echo json_encode($datos);
        exit();
        }if (isset($_GET['preview'])) { 
        $sql = "SELECT * FROM `actividades` ORDER BY id_actividad DESC LIMIT 3";
