  ...
  if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS'){
    ...
    exit(0);
        ...
        if(isset($_GET['id_docente'])){ 
          ...
          $resultado = mysqli_query($conexion,$sql) or die ( "Algo ha ido mal en la consulta a la   base de datos");
          while ($grupo = mysqli_fetch_array($resultado, MYSQL_NUM)) {
            $sqls = " SELECT avisos.id_avisos,avisos.fecha,avisos.nombre,avisos.id_grupo from avisos where id_grupo = '".$grupo[0]."' UNION "+$sqls;
        }
        $resultado1 = mysqli_query($conexion,$sqlss) or die ( "Algo ha ido mal en la consulta a la   base de datos avisos id_docente 1");
